        ...
        {QUALITY_TIERS.map((tier) => {
          ...
          return (
            <button key={tier.id} type="button" onClick={() => onChange(tier.id)}
              ...
              className={`min-w-0 rounded-xl border p-2.5 text-left transition-colors ${selected
                ? "border-amber-400/60 bg-amber-500/10 text-amber-100"
                : "hairline bg-elevated text-zinc-400 hover:bg-white/5"}`}>
              <Icon className={`mb-2 h-4 w-4 ${selected ? "text-amber-300" : "text-zinc-500"}`} />
              ...
              <div className="mt-2 text-[9px] font-mono leading-relaxed text-zinc-500">{recipeSummary(recipe)}</div>
            </button>
          );
        })}
      </div>
      <div className="border-t hairline px-4 py-2 text-[10px] text-zinc-500">
